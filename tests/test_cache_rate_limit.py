...
def test_캐시가_세지_않는_항목은_비교에서_빠진다():
    """★성원님이 더한 last_failure_at 은 ★날짜다. 카운터로 못 센다.

    비교에 끼면 매번 "다르다" 가 찍혀 로그가 못 쓰게 되고,
    정작 봐야 할 카운터 차이를 못 보게 된다.
    """
    from app.db import CACHED_COUNTERS
    assert "last_failure_at" not in CACHED_COUNTERS
    assert "session_suspicious_10m" not in CACHED_COUNTERS
    assert set(CACHED_COUNTERS) == {
        "ip_challenges_1m", "session_challenges_10m",
        "session_failures_10m", "session_telemetry_failures_10m",
    }


def test_캐시모드에서도_다른_항목이_안_사라진다(monkeypatch):
    """★통째로 갈아치우면 session_suspicious_10m·last_failure_at 이 사라져
    main.py 가 KeyError 로 터진다. 덮어쓰기여야 한다."""
    import datetime as _dt
    from app.db import CACHED_COUNTERS

    db_values = {
        "ip_challenges_1m": 1, "session_challenges_10m": 2,
        "session_failures_10m": 3, "session_telemetry_failures_10m": 4,
        "session_suspicious_10m": 9, "last_failure_at": _dt.datetime(2026, 8, 13),
    }
    cache_values = {k: 100 for k in CACHED_COUNTERS}
    merged = {**db_values, **cache_values}          # db.py 가 하는 것과 같은 식
    assert merged["session_suspicious_10m"] == 9, "★캐시가 안 세는 항목이 사라졌다"
    assert merged["last_failure_at"] == _dt.datetime(2026, 8, 13)
    assert merged["ip_challenges_1m"] == 100, "캐시 값으로 덮이지 않았다"
