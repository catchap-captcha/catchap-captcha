...
function CaptchaApp() {
  ...
  // 수집 실험용(2026-08-08, 조성원). 참여자 코드가 있으면 켜지고, 없으면 기존 동작 그대로다.
  //
  ...
  // 재생을 정면으로 가른다.
  //
  // 켜기: ?participant=<코드>  (참여자 코드가 있으면 자동으로 켜진다)
  //
  // 왜 자동인가: 예전엔 `?aim=1` 을 따로 붙여야 켜졌다. 그런데 이 구간을 빼먹고
  // 수집하면 12점짜리 드래그만 더 모이고, 그건 이미 재생 판별이 불가능하다고
  // 측정된 데이터다(AUC 0.516). 사람들의 시간을 쓰고 나서야 "못 쓴다" 가 나온다.
  // 수집 참여자에게는 늘 필요한 값이므로 코드가 있으면 켜는 것을 기본으로 둔다.
  // `?aim=0` 으로 끌 수 있다 — 조준 없이 비교군을 받아야 할 때를 위해 남겨둔다.
  //
  // 참여자 코드가 없는 일반 사용자는 예전과 완전히 같다. 켜지지 않는다.
  //
