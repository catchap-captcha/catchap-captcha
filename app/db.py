...
class Database:
    ...
    def request_pattern(self, session_id: str, client_ip_hash: str) -> dict[str, int]:
        ...
        #   판정은 여전히 DB 값(db_values)으로 한다. 캐시가 죽어도 아무 일도 안 일어난다.
        #   mode=cache 로 바꾸면 그때부터 캐시 값으로 판정한다 — 비교가 쌓인 뒤에.
        if self.cache.enabled:
            ...
            compare_and_log(db_values, cache_values)
    ...
    def record_attempt(self, challenge_id: str, selected: list[str], correct: bool, reason: str | None,
        ...
        with self.connection() as conn, conn.cursor() as cur:
            ...
            #   이미 열려 있는 커서로 한 번만 더 읽는다 — 질의를 새로 열지 않는다.
            session_id = ""
