...
class Settings:
    ...
    # ── PoW 비용 — ★실측값이다. 예전 주석의 값은 틀렸다 (2026-08-14)
    #
    # 위젯은 손으로 짠 자바스크립트 sha256 을 쓴다(`catchapGuardPow.js`). 네이티브
    # 암호 라이브러리가 아니다. 데스크톱 크로미움에서 재보니 **초당 477,453회**다.
    #
    #     비트    기대 시간      예전 주석
    ...
    #
    # 예전 값은 네이티브로 잰 것으로 보인다. 그대로 두면 읽는 사람이 안전한 값이라고
    # 착각한다 — 실제로는 24비트가 **35초** 다. 화면이 멈춘 것처럼 보이는 35초다.
    #
    # ★비용의 비대칭에 주의. 봇은 대개 네이티브 암호를 쓰므로 우리 위젯보다 훨씬 빠르다
    #   (0814 실측: Node 네이티브로 17비트를 즉시). 즉 난이도를 올릴수록 **사람이 더
    #   많이 아프다.** 그래서 상한을 사람이 견딜 수 있는 선에 둔다.
    pow_enabled: bool = os.getenv("POW_ENABLED", "1") == "1"
    pow_difficulty_bits: int = int(os.getenv("POW_DIFFICULTY_BITS", "17"))
    # 적응형 PoW: 최근 실패/과다요청 세션엔 난이도를 올린다.
    #
    # ★2 다(예전 4). `POW_STEPUP_FAILURES=1` 이라 **한 번만 틀려도** 올라가는데,
    #   사람 오답률이 12.8% 이라 여덟 명 중 한 명이 여기 걸린다. 4를 쓰면 그 사람이
    #   다음 캡차에서 4.4초를 기다린다. 2면 1.1초다.
    pow_stepup_bits: int = int(os.getenv("POW_STEPUP_BITS", "2"))
    ...
    # ★21 이다(예전 24). 24비트는 실측 35초이고, 그 대상은 **의심이 붙은 채로 3회 틀린
    #   사람**인데 우리 판정에는 오탐이 있다(특정 참가자 14.1%). 오탐을 맞은 사람이
    #   35초 멈춘 화면을 보는 것은 벌이 아니라 고장으로 보인다.
    #   21비트 = 4.4초. 봇에게는 여전히 판당 비용이고, 사람은 기다릴 수 있다.
    pow_max_bits: int = int(os.getenv("POW_MAX_BITS", "21"))
